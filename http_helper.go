...
func HttpGetWithRetry(t *testing.T, url string, tlsConfig *tls.Config, expectedStatus int, retries int, sleepBetweenRetries time.Duration) {
	err := HttpGetWithRetryE(t, url, tlsConfig, expectedStatus, retries, sleepBetweenRetries)
	if err != nil {
		t.Fatal(err)
	}
}
